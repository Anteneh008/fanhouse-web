...
export default async function CreatorDashboardPage() {
  ...
  const [postsCount, subscribersCount, mediaCount] = await Promise.all([
    db.query('SELECT COUNT(*) as count FROM posts WHERE creator_id = $1 AND is_disabled = false', [user.id]),
    db.query('SELECT COUNT(*) as count FROM subscriptions WHERE creator_id = $1 AND status = $2', [user.id, 'active']),
    db.query('SELECT COUNT(*) as count FROM media_assets ma INNER JOIN posts p ON ma.post_id = p.id WHERE p.creator_id = $1', [user.id]),
  ]);
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              {recentPostsResult.rows.length > 0 ? (
                <div className="space-y-4">
                  {recentPostsResult.rows.map((post: any) => (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                            <span className="capitalize">{post.visibility_type}</span>
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {profile?.subscription_price_cents
                      ? `$${(profile.subscription_price_cents / 100).toFixed(2)}/month`
                      : 'Not set'}
                  </dd>
                </div>
              </dl>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <dl className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <dd className="text-sm font-medium text-gray-900">{stats.posts}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Active Subscribers</dt>
                  <dd className="text-sm font-medium text-gray-900">{stats.subscribers}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Media Files</dt>
                  <dd className="text-sm font-medium text-gray-900">{stats.media}</dd>
                </div>
              </dl>
            </div>
